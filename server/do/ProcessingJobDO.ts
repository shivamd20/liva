...
export class ProcessingJobDO extends DurableObject<Env> {
	...
	async alarm(): Promise<void> {
		const jobs = this.sql.exec("SELECT id FROM jobs WHERE status NOT IN ('complete', 'failed') ORDER BY created_at ASC LIMIT 1").toArray() as { id: string }[];
		...
		try {
			...
			console.error("[ProcessingJobDO] alarm error for job", jobId, e);
			this.updateJob(jobId, { status: "failed", error: errMsg });
		}
	}
	...
	private async runStateMachine(job: JobRow): Promise<void> {
		...
		if (!env.MEDIA_PROCESSOR) {
			this.updateJob(jobId, { status: "failed", error: "MEDIA_PROCESSOR binding not configured" });
			return;
		}

		// MEDIA_PROCESSOR binding added via wrangler; getContainer expects Container namespace
		...
		switch (status) {
			...
			case "uploading_chunks": {
				...
				try {
					...
					if (!obj) {
						...
						await this.state.storage.setAlarm(CONTAINER_BOOT_BACKOFF_MS);
						return;
					}
			...
			case "processing": {
				try {
					...
					await this.state.storage.setAlarm(PROCESSING_POLL_INTERVAL_MS);
				} catch (e) {
					this.updateJob(jobId, { max_retries: maxRetries + 1 });
					await this.state.storage.setAlarm(CONTAINER_BOOT_BACKOFF_MS);
				}
			...
			case "downloading_results": {
				...
				for (const { path, r2Key, field } of outputs) {
					try {
						...
						this.updateJob(jobId, { status: "failed", error: `Failed to download ${path}: ${e}` });
						return;
					}
				}
			...
			case "transcribing": {
				...
				// Workers AI Whisper accepts audio as number[] (0-255)
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				const transcriptResult = (await this.env.AI.run("@cf/openai/whisper-large-v3-turbo", {
					audio: audioArray,

...
import { starterCode as STARTER_CODE, referenceSolution as REFERENCE_SOLUTION } from '../../../../problems/009-maximum-subarray/java';

const API_PORT = process.env.API_PORT || '5173';
const API_BASE = `http://localhost:${API_PORT}/api/v1`;
const PROBLEM_ID = 'maximum-subarray';
...
const WRONG_ALWAYS_ZERO = `class UserSolution { public int maxSubArray(int[] nums) { return 0; } }`;
const WRONG_SUM_ALL = `class UserSolution { public int maxSubArray(int[] nums) { int sum = 0; for (int n : nums) sum += n; return sum; } }`;
const COMPILE_ERROR = `class UserSolution { public int maxSubArray(int[] nums) { return 0 }`;
const RUNTIME_ERROR = `class UserSolution { public int maxSubArray(int[] nums) { return nums[10000]; } }`;

describe('Problem 009: Maximum Subarray', () => {
    ...
    describe('Wrong Answer (WA)', () => { it('should return WA for always 0', async () => { const result = await runCode(WRONG_ALWAYS_ZERO); expect(['WA', 'PA']).toContain(result.verdict); }, 60000); it('should return WA for sum all', async () => { const result = await runCode(WRONG_SUM_ALL); expect(['WA', 'PA']).toContain(result.verdict); }, 60000); });

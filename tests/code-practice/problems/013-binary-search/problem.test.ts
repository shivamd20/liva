...
import { starterCode as STARTER_CODE, referenceSolution as REFERENCE_SOLUTION } from '../../../../problems/013-binary-search/java';
...
const WRONG_ALWAYS_MINUS_ONE = `class UserSolution { public int search(int[] nums, int target) { return -1; } }`;
const COMPILE_ERROR = `class UserSolution { public int search(int[] nums, int target) { return -1 }`;
const RUNTIME_ERROR = `class UserSolution { public int search(int[] nums, int target) { return nums[10000]; } }`;

describe('Problem 013: Binary Search', () => {
    ...
    describe('Wrong Answer (WA)', () => { it('should return WA for always -1', async () => { const result = await runCode(WRONG_ALWAYS_MINUS_ONE); expect(['WA', 'PA']).toContain(result.verdict); }, 60000); });

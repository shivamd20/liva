...
import { starterCode as STARTER_CODE, referenceSolution as REFERENCE_SOLUTION } from '../../../../problems/014-merge-two-sorted-lists/java';

const API_PORT = process.env.API_PORT || '5173';
const API_BASE = `http://localhost:${API_PORT}/api/v1`;
const PROBLEM_ID = 'merge-two-sorted-lists';
...
const COMPILE_ERROR = `class UserSolution { public ListNode mergeTwoLists(ListNode l1, ListNode l2) { return null }`; // Missing closing brace
const RUNTIME_ERROR = `class UserSolution { public ListNode mergeTwoLists(ListNode l1, ListNode l2) { return l1.next.next.next.next.next; } }`;

describe('Problem 014: Merge Two Sorted Lists', () => {

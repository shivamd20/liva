...
import { starterCode as STARTER_CODE, referenceSolution as REFERENCE_SOLUTION } from '../../../../problems/008-product-of-array-except-self/java';

const API_PORT = process.env.API_PORT || '5173';
const API_BASE = `http://localhost:${API_PORT}/api/v1`;
const PROBLEM_ID = 'product-of-array-except-self';
...
const WRONG_ALL_ZEROS = `class UserSolution { public int[] productExceptSelf(int[] nums) { return new int[nums.length]; } }`;
const COMPILE_ERROR = `class UserSolution { public int[] productExceptSelf(int[] nums) { return new int[0] }`;
const RUNTIME_ERROR = `class UserSolution { public int[] productExceptSelf(int[] nums) { return new int[] { nums[10000] }; } }`;

describe('Problem 008: Product of Array Except Self', () => {
    ...
    describe('Wrong Answer (WA)', () => { it('should return WA for all zeros', async () => { const result = await runCode(WRONG_ALL_ZEROS); expect(['WA', 'PA']).toContain(result.verdict); }, 60000); });
